...
export default {
  ...
  watch: {
    issuesPerLabel: function(issuesPerLabel) {
      ...
      const labels = Object.keys(issuesPerLabel).filter(
        label => !blacklist.includes(label)
      );
      console.log(labels);
      labels.sort(
        (a, b) =>
          issuesPerLabel[b].issues.length - issuesPerLabel[a].issues.length
      );
      ...
      for (const label of labels) {
        datasets.push({
          label,
          data: [(issuesPerLabel[label].issues.length / totalIssues) * 100],
          backgroundColor: issuesPerLabel[label].color
        });
      }
      ...
      console.log(
        labels
          .map(l => ({ label: l, count: issuesPerLabel[l].issues.length }))
          .sort((a, b) => b.count - a.count)
      );

      console.log(issuesPerLabel);
      console.log(`Total count: ${totalIssues}`);
